...
export function generateMarkdownReport(
  ...
  lines.push("## 🦡 Bundle Badger Report");
  lines.push("");
  ...
  if (hasBaseline) {
    ...
    lines.push(
      `**Total:** ${formatBytes(result.totalBefore)} → ${formatBytes(result.totalAfter)} ` +
  ...
  lines.push("---");
  lines.push("");
  lines.push("_Generated by [Bundle Badger](https://github.com/mntsB/bundle-tracker)_");
...
function getAssetStatus(asset: AssetComparison): string {
  if (asset.status === "added") return "🆕";
  if (asset.status === "removed") return "🗑️";
  if (asset.diff === 0) return "✅";
  if (asset.diff > 0) {
    // Size increased
